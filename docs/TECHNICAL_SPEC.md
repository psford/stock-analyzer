...
- Multi-series mode via `chartSeries[]` array — each entry `{ticker, label, type, data, color, dash}` where type is `'primary'|'comparison'|'benchmark'`
- Series management: `addSeries()`, `removeSeries()`, `clearBenchmarkSeries()`, `clearAllSeries()`, `isMultiSeriesMode()`, `rebuildChartSeries()`
- Okabe-Ito colorblind-safe palette (`SERIES_PALETTE[]`, 7 entries) with per-position dash patterns
- Max 5 benchmarks enforced (`MAX_BENCHMARKS`), single comparison allowed (auto-replaces)
...
| 2.65 | 2026-03-10 | **Chart Benchmarks Phase 5 — indicator state tests:** 7 new Jest tests in `chartSeries.test.js` for indicator state save/restore logic. Pure functions `saveIndicatorState()`/`restoreIndicatorState()` extracted for DOM-free testing. Tests: AC6.2 (preserves all checkbox values, restores saved not defaults, returns defaults when null), AC6.F1 (RSI checked before disable restored after re-enable), double-disable guard (second save doesn't overwrite first), re-enable clears to null, independent copy (no reference sharing). Total: 58 Jest tests passing (33 chartSeries + 25 chartCalculations). |
| 2.64 | 2026-03-10 | **Chart Benchmarks Phase 5 — save/restore indicator state:** Rewrote `disableIndicators(disabled)` in `app.js`. On disable: saves all checkbox states (`show-rsi`, `show-macd`, `show-bollinger`, `show-stochastic`, `ma-20`, `ma-50`, `ma-200`), chart type value, and `show-markers` state to `this.savedIndicatorState` before unchecking/disabling. Dims all 7 labels via `opacity-50`/`cursor-not-allowed` classes. Forces chart type to `line` (candlestick invalid for multi-series). On re-enable: restores all saved states, removes dim classes, clears `savedIndicatorState` to `null`. Subsequent disable calls while already disabled do NOT overwrite saved state (preserves original user selections). Verifies AC6.1 (controls disabled/grayed), AC6.2 (state preserved and restored), AC6.F1 (RSI reappears after remove). |
| 2.63 | 2026-03-10 | **Chart Benchmarks Phase 5 — label IDs and CSS utility classes:** Added `id` attributes to MA checkbox labels (`ma-20-label`, `ma-50-label`, `ma-200-label`) and chart-type label (`chart-type-label`) in `index.html` so `disableIndicators()` can style them consistently. Added `.opacity-50` and `.cursor-not-allowed` utility classes to `base.css` (used by `disableIndicators()` for visual dimming of disabled controls). No visual change — preparation for Task 2 save/restore logic. |
| 2.62 | 2026-03-10 | **Chart Benchmarks Phase 4 complete — index search backend:** `GET /api/indices/search?q=` endpoint in `Program.cs`: queries `IndexDefinitions` DbSet with `AsNoTracking()`, filters `ProxyEtfTicker != null`, case-insensitive Contains on IndexName/IndexCode/IndexFamily, OrderBy IndexName, Take(10). Returns flat array of `{indexId, indexCode, indexName, indexFamily, region, proxyEtfTicker}`. 200+empty for blank query, 200+empty for no match. Seeded SPY, QQQ, DIA, EWU, EWG, EWJ entries (IF NOT EXISTS guards) in `seed_index_attribution.sql`. 16 xUnit tests (`IndexSearchSchemaTests`) covering AC5.1–AC5.4, AC5.F1, AC5.F2. api.js `searchIndices()` confirmed matching flat array format — no changes needed. |
| 2.61 | 2026-03-10 | **Chart Benchmarks Phase 3 complete — toggleBenchmark and search:** `toggleBenchmark(etfTicker, label, chipEl)` fetches via `API.getHistory`, calls `addSeries(..., 'benchmark', ...)`, toggles chip `.active` class, calls `disableIndicators(true)`, enforces max-5 via `addSeries`. Toggle-off: `removeSeries`, re-enables indicators if back to single series. `updateClearButtonVisibility()` shows/hides `#clear-benchmarks` and `#clear-all-overlays` based on series state. `setComparison`/`clearComparison` call `updateClearButtonVisibility`. `performBenchmarkSearch`/`showBenchmarkSearchResults`/`addBenchmarkFromSearch`/`hideBenchmarkSearchResults` implement typeahead with 300ms debounce, DOM-API result rendering (no innerHTML XSS), temp chip creation for non-static results. `API.searchIndices(query)` calls `GET /api/indices/search?q=`. `_benchmarkFetchInProgress` guard prevents concurrent fetches. init() wires benchmark chip clicks, Clear Benchmarks, All Clear, and search input events. AC2.1, AC2.2, AC2.3, AC2.4, AC2.F1, AC4.1–AC4.3 verified. |
| 2.60 | 2026-03-10 | **Chart Benchmarks Phase 3 — benchmark UI (HTML + CSS):** Added benchmark overlay section with 10 toggle chips (SPY, QQQ, DIA, IWM, EWU, EWG, EWJ, EFA, EEM, ACWI), benchmark search row (`#benchmark-search-input`, `#benchmark-search-results`, `#benchmark-search-loader`), and clear controls (`#clear-benchmarks`, `#clear-all-overlays`). CSS: `.chip-benchmark` (bordered transparent), `.chip-benchmark.active` (accent fill), `.benchmark-chips` (flex-wrap), `.chip-benchmark-temp` (dashed accent for search-created chips). |
| 2.59 | 2026-03-10 | **Chart Benchmarks Phase 2 complete — generic multi-series renderer:** `renderStockChart()` accepts `chartSeries` option; when `chartSeries.length > 1` all series are normalized to % change and rendered as Plotly scatter traces using `getSeriesPalette()` Okabe-Ito colors/dashes. Zero baseline (dotted gridColor line at y=0), horizontal legend below chart, % change y-axis. Single-stock candlestick path unchanged. Backward compatibility: builds temporary 2-entry series array from legacy `comparisonData`/`comparisonTicker` if `chartSeries` not provided. `app.js renderChart()` now passes `chartSeries: this.chartSeries` directly, clearing legacy bridge. Phase 1→2 bridge removed. AC1.2, AC1.3, AC1.4, AC3.1–AC3.4 verified. |
| 2.58 | 2026-03-10 | **Chart Benchmarks Phase 2 — theme-aware palette:** `getThemeColors()` extended with `seriesPrimary`, `seriesComparison`, `benchmark1`–`benchmark5` fields reading CSS variables. New `getSeriesPalette()` method returns `[{color, dash}]` array matching SERIES_PALETTE positions, using theme-aware colors. Enables full theme override of multi-series chart colors. |
| 2.57 | 2026-03-10 | **Chart Benchmarks Phase 1 complete — scalar removal and analyzeStock init:** `analyzeStock()` calls `rebuildChartSeries()` after `historyData` is set. `triggerReanalysis()` uses `chartSeries.find()` + `clearComparison()` instead of direct scalar mutation. `attachDragMeasure()` callbacks use `isMultiSeriesMode()` and `chartSeries` lookups. `extendChartRange()` mutates `comparisonSeries.data` directly. Zero remaining `this.comparisonTicker` / `this.comparisonHistoryData` references — Phase 1 scalar elimination complete. 51/51 tests pass. |
| 2.56 | 2026-03-10 | **Chart Benchmarks Phase 1 — renderChart bridge and scalar removal:** `renderChart()` now calls `rebuildChartSeries()` at entry and derives `comparisonData`/`comparisonTicker` from `chartSeries` instead of removed scalar state (bridge for Phase 2 charts.js refactor). `analyzeStock()` calls `rebuildChartSeries()` after `historyData` is set to initialize primary series. `triggerReanalysis()` uses `chartSeries.find()` and `clearComparison()` instead of direct scalar mutation. `attachDragMeasure()` callbacks use `isMultiSeriesMode()` and `chartSeries` lookup instead of scalar reads. `extendChartRange()` updates `comparisonSeries.data` directly instead of `this.comparisonHistoryData`. Zero remaining `this.comparisonTicker` / `this.comparisonHistoryData` raw references — Phase 1 scalar removal complete. All 51 Jest tests pass. |
...
| 2.53 | 2026-03-10 | **Chart Benchmarks Phase 1 — Data Series Abstraction:** Replaced scalar `comparisonTicker`/`comparisonHistoryData` state with generic `chartSeries[]` array in `app.js`. Each entry: `{ticker, label, type, data, color, dash}` with type `'primary'\|'comparison'\|'benchmark'`. Okabe-Ito colorblind-safe palette (7 entries) with per-position dash patterns. Series management methods: `buildPrimarySeries()`, `rebuildChartSeries()`, `assignPalettePositions()`, `addSeries()`, `removeSeries()`, `getSeriesByType()`, `clearBenchmarkSeries()`, `clearAllSeries()`, `isMultiSeriesMode()`. Max 5 benchmarks enforced. 26 Jest unit tests (`chartSeries.test.js`) covering AC1.1 structure validation and AC1.F2 max benchmark enforcement. `setComparison()` migrated to use `addSeries()`, `clearComparison()` uses `removeSeries()` + `isMultiSeriesMode()` check, `clearAll()` uses `clearAllSeries()`. |
