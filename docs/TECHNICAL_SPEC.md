...
- `GetDistinctDatesAsync(startDate, endDate)` - Get all dates with price data (recursive CTE skip-scan on `IX_Prices_EffectiveDate`)
...
- **Distinct dates (coverage):** Recursive CTE with `CROSS APPLY TOP 1` skip-scan on `IX_Prices_EffectiveDate` (~500 seeks for 2 years vs full index scan of 5M+ rows)
